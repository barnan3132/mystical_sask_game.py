	...
	if ans == "y":
		...
		if encounter < 10:
			...
			while locked == True:
				...
				print("It appears to be locked...Do you want to try and unlock it? (Y/N)")
				chest = input().lower()
				...
				while enemy.hp > 0 or player.hp > 0:
					...
					if user == "a":
						...
						if enemy.dmg > 0:
							...
							print(f"The {enemy_name}'s attack was dodged your {player.armor}!")
					if player.hp <= 0:
		...
		if village == "inn":
			...
			if inn == "sleep":
				if player.gold < 3:
					...
					print(f"You stay the night at the Tavern and heal from the hard day.")
			...
			if inn == "drink":
				if player.gold < 1:
					...
					print(f"You now have {player.gold} gold left in your pockets.")
					drunken_event = random.randint(1,100)

					if drunken_event > 10 and drunken_event <= 30:
						print(f"{player.hp} is your current health. {player.gold} is your gold.")
						...
						print("someone snatched {bad_bet} gold from your pockets")
						...
						print(" You won {good_bet} gold.\nYou now have {player.gold} gold!")
					break

		elif village == "coop" or village == "co-op":
			...
			print("Would you like to go left, right or straight? (clothing/Smith/Locksmith)")
			market = input().lower()
			if market == "clothing" or market == "left":
				print("The clothing store is stocked with all sorts of clothes.")
				...
				steel_price = 25
				if ans == "y":
					print(f"The cashier says he currently has {clothing} in stock.")
					print("Would you like to buy one?(Y/N)")
					ans2 = input().lower()

					if ans2 == "y":
						print(f"The {clothing[0]} costs {leather_price}")
						print(f"{clothing[1]} costs {steel_price}.\nleather or steel?")
						ans3 = input().lower()
						...
						if ans3 =="leather":
							if player.gold < leather_price:
								...
								print(f"The cashier hands you the Leather jacket. Your armor has increased!")
								print(f"You now have {player.gold} gold.")
								player.ac = 1
						...
						if ans3 =="steel":
							if player.gold < steel_price:
								...
								print(f"The cashier hands you the Steel jacket. Your armor has increased!")
								print(f"You now have {player.gold} gold.")
								player.ac = 2
						...
						print("The cashier turns his back and asks you to come back next time.")
						continue
				elif ans == "n":
					print("The cashier turns his back and asks you to come back next time.")
					continue

			if market == "weapons" or market == "right":

				if player.upg == 0:
					cost = 10

					print(f"The cashier agrees to upgrade your sword for {cost} gold.")
					print(f" You have {player.gold} gold. Do you want him to upgrade your blade? (Y/N)")
					upgrade = input().lower()
					if upgrade == "y":
						if player.gold < cost:
							print("You do not have enough gold!")
							...
							player.gold = player.gold - cost
							print("The cashier upgraded your sword. You now do more damage!")
			...
			elif market == "straight" or market == "Locksmith":
				print("The locksmith asks if you'd care to browse his sets. (Y/N)")
				locksmither = input().lower()

				if locksmither == "y":
					price = 20
					lockings = ["lockpick set",]
					print(f"The locksmither tells you he currently has a {lockings} in stock.")
					print(" They cost {price} gold. Would you like to buy one? (Y/N)")
					ans = input().lower()
					if ans =="y":
				...
				print('You enter the lowkey town church,\n"Health?" He asks. (Y/N)')
				ans = input().lower()
				if player.hpupg == 0:
					price = 15
				elif player.hpupg == 1:
